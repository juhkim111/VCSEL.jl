...
# vcselect!(vcm1; penfun=L1Penalty(), λ=5.0)
# println(vcm1.Σ)
# σ2_est,= vcselect(y1, V; penfun=L1Penalty(), λ=5.0, σ2=[0.5, 0.5, 0.5, 0.5, 0.5, 1.0])
# println(σ2_est)
# @testset begin
#   @test vcm1.Σ == σ2_est
# end 
...
# Σinit = [ones(d, d) for i in 1:nvarcomps]
# Σinit[end] = Matrix(I, d, d)
# Σ̂, = vcselect(Y, X, V; penfun=L1Penalty(), λ=25.0, Σ=Σinit)
# println(vcm.Σ)
# println(Σ̂)
# @testset begin
# @test Σ̂ == vcmreml.Σ
